...
Dim strSQL1, strSQL2, email  'SQL command to execute
Dim I, J, sentOn				'Loop counter
Dim SentEmail       'Currently sent email 

Server.ScriptTimeout = 200000

'strSQL = "Select distinct email from pm_emails where email = 'webmaster@torontovka.com'"
strSQL = "Select distinct email from pm_emails"
...
if ( Request.Form("action") = "mail") then
 ...
 Function return_body (txtBody, login, passwd)
	return_body = txtBody & "<TABLE WIDTH=100% CELLPADDING=7 CELLSPACING=0 BORDER=0><TR><TD align=center><p><a name=login><b>Login</b></a> = " & login & "<br> <b>Password</b> = " & passwd & "</TD></TR></TABLE></BODY></HTML>" 
 End Function
 ...
 Do while Not objPagingRS.EOF
  ...
  '			 & "If you do not wish to receive the email broadcast, then either ignore this mail or <a href=""http://www.torontovka.com/subscribe.asp?subscribe=u&email=" & objPagingRS("email") & """>click here</a> to unsubscribe</p>  </td>  </tr></table></body></html>"
	...
	Sobj.FromName = "Torontovka Dating" ' Specify sender's name
	Sobj.AddAddress  objPagingRS("email")
	Sobj.AddReplyTo "webmaster@torontovka.com"
	Sobj.IsHTML = True 
	
	Sobj.Subject = "Torontovka Five Years Anniversary @ PALAZZO"
	Sobj.Body = txtBody
